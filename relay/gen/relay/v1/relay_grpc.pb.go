...
const (
	RelayService_Publish_FullMethodName   = "/relay.v1.RelayService/Publish"
	RelayService_Subscribe_FullMethodName = "/relay.v1.RelayService/Subscribe"
	RelayService_JoinRoom_FullMethodName  = "/relay.v1.RelayService/JoinRoom"
)
...
type RelayServiceClient interface {
	...
	JoinRoom(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[RoomFrame, RoomFrame], error)
}
...
func (c *relayServiceClient) JoinRoom(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[RoomFrame, RoomFrame], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &RelayService_ServiceDesc.Streams[1], RelayService_JoinRoom_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[RoomFrame, RoomFrame]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type RelayService_JoinRoomClient = grpc.BidiStreamingClient[RoomFrame, RoomFrame]
...
type RelayServiceServer interface {
	...
	JoinRoom(grpc.BidiStreamingServer[RoomFrame, RoomFrame]) error
...
func (UnimplementedRelayServiceServer) JoinRoom(grpc.BidiStreamingServer[RoomFrame, RoomFrame]) error {
...
func _RelayService_JoinRoom_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(RelayServiceServer).JoinRoom(&grpc.GenericServerStream[RoomFrame, RoomFrame]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type RelayService_JoinRoomServer = grpc.BidiStreamingServer[RoomFrame, RoomFrame]
